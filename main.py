...
import os
os.chdir(r"C:\Users\kumar\OneDrive\Documents\Gender voice recognition")
...
if __name__ == '__main__':
    ...
    win=tk.Tk()
    win.title('Real-time Gender Recognisition System using Voice Samples')
    fnt = ('arial',20,'bold')
    titleL=tk.Label(win,text='Real-time Gender Recognisition System using Voice Samples', font = fnt)
    titleL.grid(row=1,column=1,padx=20,pady=20,columnspan=2)

    configB=tk.Button(win,text='Configuration',font = fnt,command=config)
    configB.grid(row=2,column=1,padx=20,pady=20)

    recogniseB=tk.Button(win,text='Recognize',font = fnt,command=lambda :recognise(model),state="disabled")
